...
print_('{}Neural Doodle for semantic style transfer.{}'.format(ansi.CYAN_B, ansi.ENDC))
...
print_('{}  - Using device `{}` for processing the images.{}'.format(ansi.CYAN, theano.config.device, ansi.ENDC))
...
class Model(object):
    ...
    def load_data(self):
        ...
        if not os.path.exists('vgg19_conv.pkl.bz2'):
            print_("{}ERROR: Model file with pre-trained convolution layers not found. Download here:{}\n"\
                  "https://github.com/alexjc/neural-doodle/releases/download/v0.0/vgg19_conv.pkl.bz2{}\n"\
            .format(ansi.RED_B, ansi.RED, ansi.ENDC))
            sys.exit(-1)

        data = pickle.load(file("data.pickle"))
...
class NeuralGenerator(object):
    ...
    def load_images(self, name, filename):
        ...
        if img is not None: print_('  - Loading {} image data from {}.'.format(name, filename))
        if map is not None: print_('  - Loading {} semantic map from {}.'.format(name, mapname))
    ...
    def prepare_style(self, scale=1.0):
        ...
        for layer, patches, norms in zip(self.style_layers, result[::2], result[1::2]):
            ...
            print_('  - Style layer sem{}: {} patches in {:,}kb.'.format(layer, patches.shape[0], patches.size//1000))
    ...
    def content_loss(self):
        ...
        for l, ref in zip(self.content_layers, result):
            ...
            print_('  - Content layer conv{}: {} features in {:,}kb.'.format(l, ref.shape[1], ref.size//1000))
        return content_loss
    ...
    def evaluate(self, Xn):
        ...
        py3_temp = self.compute_grad_and_losses(current_img, self.content_map)
        grads = py3_temp[0]
        losses = py3_temp[1:]
        ...
        if args.print_every and self.frame % args.print_every == 0:
            print_('{:>3}   {}error{} {:8.2e} '.format(self.frame, ansi.BOLD, ansi.ENDC, loss / 1000.0), end='')
            category = ''
            for v, l in zip(losses, self.losses):
                if l[0] == 'smooth':
                    continue
                if l[0] != category:
                    print_('  {}{}{}'.format(ansi.BOLD, l[0], ansi.ENDC), end='')
                    category = l[0]
                print_(' {}{}{} {:8.2e} '.format(ansi.BOLD, l[1], ansi.ENDC, v / 1000.0), end='')

            quality = 100.0 - 100.0 * np.sqrt(self.error / 255.0)
            print_('  {}quality{} {:3.1f}% '.format(ansi.BOLD, ansi.ENDC, quality, flush=True))
    ...
    def run(self):
        ...
        for i in range(args.resolutions):
            ...
            try:
                ...
                print_("{}ERROR: The optimization diverged and NaNs were encountered.{}\n"\
